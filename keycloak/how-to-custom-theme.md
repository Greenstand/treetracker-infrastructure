...
--name keycloak \
--rm \
quay.io/keycloak/keycloak:22.0.3 start-dev --spi-theme-static-max-age=-1 --spi-theme-cache-themes=false --spi-theme-cache-templates=false
```

# Add Theme

```
docker exec -i keycloak sh <<EOF
mkdir -p /opt/keycloak/themes/treetracker/login
cd /opt/keycloak/themes/treetracker/login

# login
cat <<EO2 > theme.properties
parent=keycloak
...
cat <<EO3 > resources/css/treetracker.css
.login-pf body {
...
cat <<EO4 > resources/js/treetracker.js
alert("Hello World!");
EO4
